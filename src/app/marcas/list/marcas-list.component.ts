...
export default class MarcasListComponent extends BaseListComponent<Marca> {
  ...
  protected override loadService(
    page: number,
    limit: number,
  ): Observable<{ data: Marca[]; total: number }> {
  ...
  protected override getId(item: Marca): number | string {
    return item._id!;
  }
  protected override getDisplayName(item: Marca): string {
    return item.nombre;
  }
}
