    ...
    <form [formGroup]="form" (ngSubmit)="submit()">
      <mat-dialog-content>
        <mat-form-field appearance="fill" class="full-width">
          ...
          <input matInput formControlName="nombre" placeholder="Nombre de la marca" autocomplete="off">
...
export class MarcaFormComponent {
  ...
  readonly form = this.fb.group({ nombre: [this.data?.nombre ?? '', Validators.required], estado: [this.data?.estado ?? 1] });
  ...
  submit(): void {
    ...
    obs.subscribe({
      next: () => { this.snackBar.open(`Marca ${this.data ? 'actualizada' : 'creada'}`, 'Cerrar', { duration: 2000 }); this.dialogRef.close(true); },
