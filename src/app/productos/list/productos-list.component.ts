        ...
        <table mat-table [dataSource]="data()" class="full-table">
          ...
          <tr class="mat-row" *matNoDataRow>
            <td class="mat-cell" [attr.colspan]="columns.length">
              <div class="empty-state"><mat-icon>inventory_2</mat-icon>
...
export default class ProductosListComponent extends BaseListComponent<Producto> {
  ...
  override columns = ['codigo', 'nombre', 'categoria', 'marca', 'presentaciones', 'estado', 'acciones'];
  ...
  protected override loadService(page: number, limit: number, search?: string): Observable<{ data: Producto[]; total: number }> {
  ...
  protected override getId(item: Producto): number | string { return item.codigoprod!; }
  protected override getDisplayName(item: Producto): string { return item.nombre; }
}
