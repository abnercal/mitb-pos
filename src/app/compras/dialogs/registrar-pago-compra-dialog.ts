...
import { RegistrarPagoCompraBody } from '../../core/services/compra.service';

export interface RegistrarPagoCompraDialogData {
  compra: Compra;
}

@Component({
  selector: 'app-registrar-pago-compra-dialog',
  standalone: true,
  imports: [
  ...
  template: `
    ...
    <mat-dialog-content>
      @if (data.compra.saldoPendiente != null) {
        <p class="saldo-hint">Saldo pendiente: <strong>Q {{ data.compra.saldoPendiente | number: '.2' }}</strong></p>
      }
...
export class RegistrarPagoCompraDialog implements OnInit {
  readonly data = inject<RegistrarPagoCompraDialogData>(MAT_DIALOG_DATA);
  private readonly dialogRef = inject(MatDialogRef<RegistrarPagoCompraDialog>);
  private readonly tipoPagoService = inject(TipoPagoService);

  tiposPago: TipoPago[] = [];
  importe: number = this.data.compra.saldoPendiente ?? 0;
  ...
  submit(): void {
    ...
    const body: RegistrarPagoCompraBody = {
